  ...
  describe "converting cinetv movie resume to RDF"
    ...
    $ do
        ...
        RDF.triplesOf graph `shouldContainElems` catMaybes
          ...
          , RDF.mkTripleLit synopsisFrenchUri SW.crmP190 "Résumé en français"
          ...
          , RDF.mkTripleLit synopsisEnglishUri SW.crmP190 "Resume in english"
...
dbSetup = do
  ...
  _    <- liftIO $ flip liftSqlPersistMPool pool $ do
    ...
    insertKey (toSqlKey 50)
      $ FilmoResumes (toSqlKey 1) "Résumé en français"
    insertKey (toSqlKey 100)
      $ FilmoResumesAnglais (toSqlKey 1) "Resume in english"
  return pool
